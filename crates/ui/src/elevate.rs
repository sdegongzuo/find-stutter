//! UAC 自动提权（Windows ShellExecuteExW + "runas"）。
//!
//! ## 用途
//!
//! GUI 是普通用户身份启动的，但 `find-stutter-service install` / `start` 需要管理员权限。
//! 首次部署时让 GUI 自动检测到「服务未注册」并弹 UAC 申请 admin，完成 install + start，
//! 用户体验是「双击 GUI → 一路点是 → 看到绿点」。
//!
//! ## 实现
//!
...
//! - 非 Windows: 直接返回 `ElevateOutcome::Unsupported`（Linux/macOS 应另走 `pkexec`/`osascript`）。
//!
//! ## 失败模式
//!
//! - `UacDenied`：用户在 UAC 弹窗里点「否」→ 1223
//! - `ExeNotFound`：service.exe 不存在
...
pub enum ElevateOutcome {
    ...
    ShellFailed(i32),
    /// 子进程未在 `timeout` 内退出（服务 hang 住；GUI 不应无限等）
    Timeout,
...
///
/// - `exe`：`find-stutter-service.exe` 的完整路径
...
mod tests {
    ...
    /// 验证：wide_chars 已删除（改用 encode_utf16 内联）
    #[test]
    fn wide_chars_removed() {
        // 占位测试：原 wide_chars 已被 encode_utf16 内联取代
    }
    ...
    /// 验证：非 Windows 平台（在 cross-compile / cargo test 默认 target 上）
    ///       路径不存在时也直接返回 Unsupported，因为 cfg!(windows) 为 false
    #[test]
    #[cfg(not(windows))]
